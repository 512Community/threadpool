...
#define POOL(ptr, type, number) ((ptr)->type##_info.number)
	...
	pthread_mutex_t busy_thr_num_lock;	/* 用于使用忙线程数时的锁 */
...
int threadpool_create(void **__pool, unsigned int min_thr_num, unsigned int max_thr_num, unsigned int queue_max_size)
{
	...
	if (pthread_mutex_init(&(pool->lock), NULL) || pthread_mutex_init(&(pool->busy_thr_num_lock), NULL)) {
...
int threadpool_add_task(void *_pool, void *(*function)(void *arg), void *arg)
{
	...
	/*如果队列满了,调用wait阻塞*/
	while ((queue_size == queue_max_size) && (!pool->shutdown))
...
static void *threadpool_thread(void *threadpool)
{
	...
	for (;;) {
		...
		if (pool->shutdown) {
			pthread_mutex_unlock(&(pool->lock));
			ESLOG_ERR("thread 0x%x is exiting \n", (unsigned int)pthread_self());
			return NULL;
		}
